...
from .common import _recent_event, _status_default
...
def get_status():
    ...
    try:
        with state.state_lock:
            tx = bool(getattr(state.tx_state, "transmitting", False))

            # Prefer rx_db if you’ve added it; fallback to current_rms
            rx_db = getattr(state.meter, "rx_db", None)
            if rx_db is None:
                current_level = float(state.repeater_state.current_rms or 0.0)
                if current_level > 0:
            ...
            if state.ptt_manager:
                status_text, status_color = state.ptt_manager.get_ptt_status()
            ...
            tot_lockout = (
                bool(state.tot_manager.is_locked()) if state.tot_manager else False
            )

            tot_elapsed = 0.0
            lockout_remaining = 0.0
            if tot_enabled and state.tot_manager:
                tm = state.tot_manager
                ...
                if tot_lockout and getattr(tm, "lockout_start", None):
                    ...
                    lockout_remaining = max(
                        0.0, lockout_time - float(now - tm.lockout_start)
                    )

            # ID
            sending_id = bool(getattr(state.schedule_id, "sending_id", False))
            next_id_in = None
            ident = cfg.get("identification", {})
            if bool(ident.get("cw_enabled", False)) and state.schedule_id:
                interval = float(ident.get("interval_minutes", 10)) * 60.0
                last_id = float(getattr(state.schedule_id, "last_id_time", now) or now)
        ...
        return jsonify(
            {
                ...
                "next_id_in": float(round(next_id_in, 1))
                if next_id_in is not None
                else None,
                "clipping": clipping,
                "limiting": limiting,
            }
