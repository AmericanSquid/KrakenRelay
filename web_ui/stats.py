    ...
    try:
        ...
        for line in out.splitlines():
            if match_token in line:
                return float(line.split()[index].replace("+", "").replace("°C", ""))
    except Exception:
        pass
    return None
...
def _cpu_temp_vcgencmd() -> Optional[float]:
    try:
        out = subprocess.check_output(
            ["vcgencmd", "measure_temp"], text=True, timeout=0.5
        )
        # temp=65.2'C
...
def mumble_status():
    try:
        result = subprocess.run(
            ["systemctl", "is-active", "mumble-cli"], capture_output=True, text=True
        )
