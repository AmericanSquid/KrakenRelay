...
class AudioDeviceManager:
    ...
    def create_input_stream(
        self, device_index, format=pyaudio.paInt16, rate=48000, chunk=1024
    ):
        pa = self.pa
        try:
    ...
    def create_output_stream(
        self, device_index, format=pyaudio.paInt16, channels=1, rate=48000, chunk=1024
    ):
        pa = self.pa
        try:
            ...
            logging.info(
                f"Created output stream: rate={actual_rate}, channels={channels}"
            )
            return stream
        except Exception as e:
