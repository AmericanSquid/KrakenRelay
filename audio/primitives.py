...
def partial_linear_gain_ramp(
    ...
    start_pos = int(max(0, start_pos))
    length = int(length)
    total_length = int(max(1, total_length))

    idx = np.arange(start_pos, start_pos + length, dtype=np.float32)
    denom = max(1, total_length - 1)
    progress = np.clip(idx / float(denom), 0.0, 1.0)

    return (
        float(start_gain) + ((float(end_gain) - float(start_gain)) * progress)
    ).astype(np.float32)
