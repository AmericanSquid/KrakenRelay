...
def carrier_validity_probe(
    ...
    if delta >= cv_min_delta:
        ...
        if not state._carrier_valid:
            if debug_enabled():
                logging.debug(
                    "[CarrierValidity] VALID delta=%.3f >= %.3f", delta, cv_min_delta
                )
    ...
    if _probe_expired(state, now, cv_timeout):
        ...
        if audit:
            audit.warning(
                event_type=AuditEvent.CARRIER_VALIDITY_FAILED,
                source="carrier_validity",
                message="Carrier validity probe failed; dropping squelch",
                metadata={
                    "delta": round(delta, 3),
                    "min_delta": round(cv_min_delta, 3),
                    "timeout": round(cv_timeout, 2),
                    "raw_level_db": round(float(raw_level_db), 2),
                    "was_transmitting": bool(tx.transmitting),
                    "kerchunk_buffer_len": len(state.kerchunk_buffer),
                },
            )
